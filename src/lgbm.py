...
for col in ["item_nbr","store_nbr","cluster"]:
...
val_pred=[]
test_pred=[]
#cate_vars = ['item_nbr','store_nbr','class','state','type','cluster','family']
cate_vars=["item_nbr","store_nbr","cluster"]
...
for i in range(16):
    ...
    dtrain = lgb.Dataset(
        ...
        weight=pd.concat([items["perishable"]] * noOfWeeks) * 0.25 + 1
    )
    dval = lgb.Dataset(
...
plt.savefig("summary_plot_lgbm.png",bbox_inches="tight")

feature_importance_df.to_csv('lgb_features.csv',float_format='%.4f', index=None)
